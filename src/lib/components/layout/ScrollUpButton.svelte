...
<script lang="ts">
  ...
  function handleScroll() {
    ...
    // If the scroll up button is null,
    // or if the window isn't defined, immediately exit the function
    if (scrollUpButton == null || typeof window === "undefined") return;

    // Gets the current scroll height
    const currentScrollHeight = window.scrollY;

    // If the current scroll height is greater than the
    // scroll height threshold and if the current scroll height
    // is less than the previous scroll height,
  ...
  onMount(() => {
    ...
    // Adds the event listener for the scroll event to the document
