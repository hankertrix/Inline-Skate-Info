  ...
  function getTableOfContents(headings: NodeListOf<Element>) {
    ...
    for (const heading of headings) {
      ...
      if (firstHeadingLevel < 0) {
        ...
        // Otherwise, if the current heading level is lower than that of the first heading level, throw an error
      else if (currentHeadingLevel < firstHeadingLevel) {
        throw Error(`Failed to generate table of contents as the heading "${heading.textContent}" has a smaller heading level (${`h${currentHeadingLevel}`}) than the first heading level (${`h${firstHeadingLevel}`}).`);
      }

      // Initialise the previous heading variable
      let previousHeading = null;

      // If the current heading level is greater than the previous heading level
      if (currentHeadingLevel > previousHeadingLevel) {
        ...
        // If the current heading level is the same or less than that of the previous heading
        ...
        // Gets the previous heading has a level lower than the current heading level
        // Iterates backwards while the level is greater or equal to zero and the previous heading exists
        while (level >= 0 && previousHeading == null) {

          // Gets the previous heading reference from the list of previous headings
