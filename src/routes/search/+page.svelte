...
<script lang="ts">
  ...
  let { searchTerm, results } = $derived(data);

  // Gets the number of results on the page
  let numberOfResults = $derived(results.length);
</script>
...
{#if numberOfResults < 1}
  ...
  <!-- Show that there are no results found -->
  <!-- and ask the user to try a different search term -->
