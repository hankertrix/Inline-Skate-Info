...
<script lang="ts">
  ...
  const supportedRetailers = Object.fromEntries(
    Object.entries(retailersInSingapore).slice(...discountInfoJson.listSlice)
  );
  ...
  <section class="catalogue">
    ...
    <div class="link-wrapper">
      ...
      {#each discountInfoJson.pdfFiles as file}
        ...
        <a href={convertFilePathToUrl(file)} target="_blank" title="View the discounted skates"
          >{getFilenameFromFilePath(file)}</a
        >
      {/each}
    </div>
  </section>
