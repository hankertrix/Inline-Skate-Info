...
const ListsPage = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `      ${$$result.head += `<!-- HEAD_svelte-1n1sarj_START -->${$$result.title = `<title>${escape(title)} - Inline Skate Info</title>`, ""}<!-- HEAD_svelte-1n1sarj_END -->`, ""}  <main class="svelte-zadhlh"><h1 class="text svelte-zadhlh"${add_attribute("id", makeUrlFriendlyString(title), 0)}>${escape(title)}</h1> <div><header> <p class="text svelte-zadhlh">${escape(heading)}</p></header>  ${listsJson.links.length > 0 ? (() => {
