import { B as BASE_PATH, S as SPACING, M as MAX_CHARACTERS, C as CACHE_TIME, E as EXIT_MESSAGE, a as MESSAGE_ENTITY_LIMIT, O as OPERATION_CANCELLED_MSG, D as DEV, b as CATEGORY_SPACING$1, L as LABEL_SPACING$1 } from "../../../chunks/constants.js";
import { Scenes, Composer, Telegraf, session } from "telegraf";
import { readFile, readdir } from "node:fs/promises";
import * as pathLib from "path";
import * as filters from "telegraf/filters";
const ENABLE_SETTING_WEBHOOK = true;
...
function convertStaticFilePathToUrl(path) {
  if (!path.startsWith("./static/")) {
    ...
    return `${BASE_PATH}${path.replace(/\.\/static/g, "")}`;
}
...
function debounce(func, timeInMs = 2e3) {
  let timer;
...
function unstripValidHtmlTags(text) {
...
const trainingLocation$1 = hyperlink("NTU MPC 1", "https://goo.gl/maps/uwQk8Q38B4Rdv18B8");
...
async function handler$5(ctx, message) {
...
async function handler$4(ctx, msg) {
  if (!msg)
  ...
  if (msg.length > 50) {
    ...
    await deleteMessages(ctx, ctx.message.message_id);
    return await callback2(ctx, message2);
...
async function handler$3(ctx, message) {
...
async function qrCodeHandler(message) {
...
${hyperlink("Here", BASE_PATH)} is the bot's website if you prefer to view the information presented here on a website.
...
bot.command([
  ...
  const [trickName, trickText] = await handler$1(ctx.message.text);
  const reply = `${bold(trickName)}${SPACING}${trickText}`;
  await ctxReply(ctx, reply);
});
bot.inlineQuery(regex$3, async (ctx) => {
  const [trick, reply] = await handler$1(ctx.inlineQuery.query);
...
bot.command([
  ...
  const reply = await handler$2(ctx.message.text);
  await ctxReply(ctx, reply);
});
bot.inlineQuery(regex$6, async (ctx) => {
  const reply = await handler$2(ctx.inlineQuery.query);
...
bot.command([
  ...
  const msgText = ctx.message.text;
  const [_, qrCodeDataURL] = await qrCodeHandler(msgText);
  if (qrCodeDataURL)
    return await ctx.replyWithPhoto({ source: Buffer.from(qrCodeDataURL, "base64") });
  async function callback(ctx2, input) {
    const [_2, qrCodeDataURL2] = await qrCodeHandler(input);
    return await ctx2.replyWithPhoto({ source: Buffer.from(qrCodeDataURL2, "base64") });
  }
  await ctx.scene.enter("validate", { message: "Please enter the text you want to convert to a QR code.", callback });
});
bot.inlineQuery(qrCodeRegex, debounce(async (ctx) => {
  const queryText = ctx.inlineQuery.query;
  const [message, qrCodeDataURL] = await qrCodeHandler(queryText);
  if (!qrCodeDataURL)
    return;
