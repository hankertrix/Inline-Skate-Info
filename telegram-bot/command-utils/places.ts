// Module to handle all the commands related to places,
// like where to buy skates, where to rent skates and so on
...
export const whereToBuyRegex = new RegExp(
  String.raw`^\/?\b(?:(?:(?:places?|where)(?:[ _-]?to[ _-]?buy))|buy(?:ing)?)\b(?:${utils.regexEscape(BOT_USERNAME)})?`,
  "i"
);

// The regex to check for the where to rent command
export const whereToRentRegex = new RegExp(
  String.raw`^\/?\b(?:(?:(?:places?|where)(?:[ _-]?to[ _-]?rent))|rent(?:ing)?)\b(?:${utils.regexEscape(BOT_USERNAME)})?`,
  "i"
);

// The regex to check for the skate parks command
export const skateParksRegex = new RegExp(
  String.raw`^\/?\b(?:skate)?[ _-]?(?:park|ramp)s?\b(?:${utils.regexEscape(BOT_USERNAME)})?`,
  "i"
);

// The regex to check for the skating rinks command
export const skatingRinksRegex = new RegExp(
  String.raw`^\/?\b(?:skat(?:e|ing))?[ _-]?rinks?\b(?:${utils.regexEscape(BOT_USERNAME)})?`,
  "i"
);
...
// The spacing between the title and the rest of the content.
// It includes the places spacing as well,
// so the effective title spacing would be PLACES_SPACING + TITLE_SPACING
const TITLE_SPACING = "";
...
export async function generatePlacesText(
  place: Places,
  formatFunc: (text: string) => string = utils.titlecase,
  separator: string = ": "
) {
  //
  ...
  for (const [placeName, placeInfo] of Object.entries(json)) {
    ...
    // If the place has a website, adds the bolded
    // and hyperlinked place name to the list
    if (typeof placeInfo.website === "string") {
      placeInfoStrList.push(
        utils.bold(utils.hyperlink(placeName, placeInfo.website))
      );
    }

    // Otherwise, just add the place name in bold
    ...
    for (const [key, value] of Object.entries(placeInfo)) {
      ...
      const label = utils.bold(`${utils.titlecase(key)}${separator}`);

      // If the current label is the address and
      // there is a map link in the place information
      if (key === "address" && typeof placeInfo.mapLink === "string") {
        //

        // Adds the address hyperlinked with the map link
        placeInfoStrList.push(
          `${label}${utils.hyperlink(value, placeInfo.mapLink)}`
        );
      }
...
export async function whereToBuyHandler(message: string) {
  ...
  // Otherwise, if the message is some other words
    ...
    // sets the list of JSON files to the one in the category
    if (category) {
      jsonFiles = [category];
    }
  ...
  const replyStrList = await Promise.all(
    jsonFiles.map((place) => generatePlacesText(place))
  );
