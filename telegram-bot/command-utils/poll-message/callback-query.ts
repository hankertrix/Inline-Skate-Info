...
import { type PollConfig, POLL_TYPES, createConfig } from "./config";
...
function getIndexOrDefaultValue(index: number, defaultValue: number = 0) {
...
export function getPollOptionSegment(message: string, pollOption: string) {
	...
	const pollOptionNameSegment = message
		.slice(newLineAfterPollOptionIndex, doubleNewLineIndex)
		.trim();
...
export function regeneratePollPortion(
	...
	const { pollOptionNameSegment, pollOptionLine } = getPollOptionSegment(
		message,
		pollOption,
	);
	...
	const matches = Array.from(pollOptionNameSegment.matchAll(regex));
...
export function reformPollMessage(
	...
	for (const pollOption of pollOptions) {
		...
		const { pollPortion, names, nameRemoved, nameTagged } =
			regeneratePollPortion(
				message,
				pollOption,
				isSelectedPollOption,
				pollConfig,
				isSelectedPollOption || pollConfig.isSingleChoicePoll
					? name
					: null,
				tagString,
			);
	...
	const respondedPortion = createNumberOfPeoplePortion(
		numResponded,
		pollConfig.formatOptions.messageFooter,
		pollConfig,
	);
