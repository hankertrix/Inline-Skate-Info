...
async function doneCommandHandler(ctx: Scenes.WizardContext) {
	...
	if (!state.pollMessage || !pollConfig.pollOptions?.length) {
		...
		return await promptUserForInput(ctx, pollConfig.incompleteDataMessage);
	}
	...
	await deleteMessages(ctx, ...(state.messagesToDelete ?? []));
...
export function getNumberingStyleFromString(str: string) {
	return str
		.replace(getNumberingStyleFromStringRegex, "")
...
export const createPollMessageScene = new Scenes.WizardScene(
	...
	createStep(
		async (context: Scenes.WizardContext, next: () => Promise<void>) => {
			...
			// Gets the poll config object
			const pollConfig =
				state.pollConfig as Required<CreatePollMessageConfig>;
			...
			if (!message) {
				...
				return await promptUserForInput(ctx, pollConfig.prompts[0]);
			}
	...
	createStep(async (context: Scenes.WizardContext) => {
		// Cast the context to the correct type
		const ctx = context as CreatePollMessageContext;
		...
		if (!numberingStyles.includes(message)) {
			...
			return await promptUserForInput(
				ctx,
				utils.strFormat(prompts.failure.prompt, {
		...
		// Otherwise
			...
			await promptUserForInput(ctx, prompts.success.prompt, {
				...generateReplyKeyboard(["Yes", "No"], {
					oneTime: true,
					resize: true,
	...
	createStep(async (context: Scenes.WizardContext) => {
		...
		if (!messageIsValid) {
			...
			return await promptUserForInput(ctx, prompts.failure.prompt, {
				...generateReplyKeyboard(["Yes", "No"], {
					oneTime: true,
					resize: true,
					placeholder: prompts.failure.placeholder,
		...
		await promptUserForInput(ctx, prompts.success, {
	...
	createStep(
		async (context: Scenes.WizardContext, next: () => Promise<void>) => {
			...
			if (!message) {
				...
				// and exit the function
				return await promptUserForInput(ctx, prompts.failure);
			}
			...
			// Otherwise
				...
				if (!pollConfig.additionalOptionsFuncList?.length) {
					...
					// and exit the function
					return await promptUserForInput(ctx, prompts.success);
				}
