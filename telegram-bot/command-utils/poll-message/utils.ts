...
export function createNumberOfPeoplePortion(
	...
	const numberOfPeoplePortion = utils
		.strFormat(formatOption.formatStr, {
			people: numberOfPeopleString,
	...
	return formatOption.bold
		? utils.bold(numberOfPeoplePortion)
...
export function createPollPortion(pollConfig: Required<PollConfig>) {
	...
	for (const [index, pollOption] of pollConfig.pollOptions.entries()) {
		...
		if (
			(pollConfig.numberingStyle && maxEntries < 1) ||
			pollConfig.preserveLines
		) {
