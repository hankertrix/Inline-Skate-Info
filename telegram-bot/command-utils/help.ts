...
function getCommandDict() {
	...
	const commandDict = {
		start: {
			explanation:
				"Starts the bot and displays a basic overview of what the bot does.",
		},
		...
		terminology: {
			explanation:
				"Displays the common terms that are regularly used in inline skating. You can search for a term or display a category of terms with this command.",
		...
		tricks: {
			explanation:
				"Displays information about inline skating tricks. You can choose to either display a specific category of tricks or search for a trick. The videos are ranked by their usefulness, so the most useful video is first and the least useful video is last.",
		...
		routes: {
			explanation:
				"Provides a list of urban skating routes for you to peruse.",
		...
		rulebooks: {
			explanation:
				"Provides a list of official rulebooks for you to peruse.",
		},
		...
		maintenance_guides: {
			explanation:
				"Provides a list of maintenance guides for you to peruse.",
		},
		...
		misc_resources: {
			explanation:
				"Provides a list of miscellaneous resources for you to peruse.",
		},

		skate_boot_types: {
			explanation:
				"Displays information about the different types of skate boots.",
		},
		...
		discount_info: {
			explanation:
				"Displays information about the tertiary student discount. This discount is only available to tertiary students studying in Singapore.",
		},

		where_to_buy: {
			explanation:
				"Displays information about the places you can buy skates from. You can display a specific category of places, such as online-only stores.",
			usage: "<category (optional)>",
			categories: ["all", "local", "overseas", "online"],
		},

		where_to_rent: {
			explanation:
				"Displays information about the places where you can rent inline skates in Singapore.",
		},
		...
		skating_rinks: {
			explanation:
				"Displays information about skating rinks in Singapore.",
		},

		brands: {
			explanation:
				"Displays information about the brands related to inline skating. You can display a specific category of brands, such as brands that only sell wheels.",
			usage: "<category (optional)>",
			categories: [
				"all",
				...Object.values(BRAND_CATEGORY).map(category =>
					category.replace(/[ _-]brands?/g, ""),
				),
			],
		},
		...
		fr_differences: {
			explanation:
				"Displays information about the differences between the various FR skates.",
		},

		f5s_vs_f6s: {
			explanation:
				"Displays information about the differences between the Flying Eagle F5S and the Flying Eagle F6S.",
		},

		triskate_differences: {
			explanation:
				"Displays information about the differences between triskates and regular 4-wheeled inline skates.",
		},

		protective_gear: {
			explanation:
				"Displays information about the protective gear you can buy.",
		},

		accessories: {
			explanation:
				"Displays information about the accessories that you can buy for inline skating.",
		},

		tools: {
			explanation:
				"Displays information about the tools you can buy to have an easier time maintaining your skates.",
		},

		maintenance_items: {
			explanation:
				"Displays information about the maintenance items you might need to maintain your skates.",
		},

		clothing: {
			explanation:
				"Displays information about the clothing you can buy, which are mostly just socks.",
		},

		poll: {
			explanation:
				"Gets the bot to send a @countmeinbot styled poll message but with only one option called 'Coming'. You can also add the bot as an admin to your group to have the bot automatically delete the messages you sent to create the poll.",
		...
		trg_msg_help: {
			explanation:
				"Displays how to use the /trg_msg command if you have set it up.",
		},
		...
		rental_msg_help: {
			explanation:
				"Displays how to use the /rental_msg command if you have set it up.",
		},

		create_poll: {
			explanation:
				"Create a @countmeinbot styled poll with custom poll options. The bot will guide you through the process of creating a poll message using this function. The bot will automatically delete the messages you sent to create the poll if you have added the bot as an admin to your group.",
			usage: "<poll message (optional)>",
		},

		create_rental_msg: {
			explanation:
				"Create a rentals-oriented @countmeinbot styled poll with custom rental options. The bot will automatically delete the messages you sent to create the rental message if you have added the bot as an admin to your group.",
		...
		get_chat_id: {
			explanation:
				"Gets the bot to send the chat ID of the chat. This command is just here to facilitate the creation of the training message and the rental message for the /trg_msg command and /rental_msg command respectively.",
		},

		source: {
			explanation:
				"View the source code for the bot. Under the GNU Affero General Public License (AGPL) Version 3, a developer is required to make the source code available to all users who use his application.",
		},
	} as const;
...
export function getCommandHelpMsg(args: {
	...
	if (args.command) {
		...
		// Change all the spaces and dashes to underscores
		...
		entry = [
			command,
			utils.dictGet(commandDict, command) as HelpInfo | null,
		];
	}
	...
	for (const [label, value] of Object.entries(info)) {
		...
		if (label === "usage") {
			...
			infoList.push(
				`${utils.bold(utils.titlecase(`${label}:`))} ${utils.monospace(
...
export function generateCommandMsg() {
	...
	for (const [command, info] of Object.entries(commandDict)) {
		...
		msgList.push(
			`${command} - ${info.explanation.match(firstSentenceRegex)![0]}`,
		);
	}
