...
export const regex = new RegExp(
	String.raw`^\/?\bbrands?\b(?:${utils.regexEscape(BOT_USERNAME)})?`,
	"i",
);
...
async function generateText(category: BrandCategory) {
	...
	brandsMsgList.push(utils.bold(utils.getTitleFromFilename(category)));
...
export async function handler(message: string) {
	...
	const category = utils.dictGet(
		brandCategories,
		msg,
	) as BrandCategory | null;
	...
	const textList = await Promise.all(
		jsonFiles.map(file => generateText(file)),
	);
