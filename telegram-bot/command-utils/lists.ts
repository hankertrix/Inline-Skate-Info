...
export const maintenanceGuidesRegex =
	/^\/?\bmain(?:tenance|tain(?:[ea]nce|ing)?)[ _-]?(?:guides?)\b/i;

// The regular expression to check for the glossaries command
export const glossariesRegex =
	/^\/?\b(?:(?:glossar(?:y|ies?)|dict?(?:ionar(?:y|ie?))?s?)(?:[ _-]?of[ _-]?term(?:s|inology|inologies?)?)?)\b/i;

// The regular expression to check for the miscellaneous resources command
export const miscResourcesRegex =
	/^\/?\b(?:misc(?:ell?an[ea]ous)?s?|ran(?:dom)?s?)?[ _-]?(?:resour?ce|guide)s?\b/i;

// Function to generate the lists text
export async function generateListsText(
	lists: Lists,
	...
	for (const [linkTitle, link] of links.values()) {
		...
		if (linkTitle.length !== 0)
			processedLink = utils.hyperlink(linkTitle, link);
