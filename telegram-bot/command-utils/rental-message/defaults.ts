...
export const DEFAULT_RENTAL_MSG_CONFIG: Required<PollConfig> = {
	...
	inlineKeyboardGenerator:
		generateRentalMsgInlineKeyboardFunc(DEFAULT_TAG_STRING),
} as const;
...
const removeTagStringRegex = new RegExp(`${regexEscape(DEFAULT_TAG_STRING)}$`);
...
export function defaultIsSameNameFunc(
	...
	return (
		encounteredName.replace(removeTagStringRegex, "").trim() === givenName
	);
}
...
export async function promptForMaxEntries(
	...
	if (!maxEntriesMatch) {
		...
		await promptUserForInput(
			ctx,
			"Please enter the number of available rentals for this option.",
		);
...
export const DEFAULT_CREATE_RENTAL_MSG_CONFIG: Required<CreatePollMessageConfig> =
	{
		...
		additionalOptionsFuncList: [promptForMaxEntries],
