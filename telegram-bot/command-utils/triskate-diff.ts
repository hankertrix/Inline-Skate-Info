...
export const regex =
	/^\/?\b(?:tri[ _-]?skates?[ _-]?(?:diff?(?:erences?)?)?|3[ _-]?vs[ _-]?4[ _-]?(?:wheels?)?|wheels?[ _-]?compar(?:e|ison)s?)\b/i;
...
export async function generateMsg() {
	...
	msgList.push(
		utils.bold("Differences between triskates and regular skates"),
	);
	...
	for (const [label, info] of Object.entries(data)) {
		...
		// Otherwise
			...
			infoList.push(utils.bold(`Relevant ${label}`));
			...
			for (const [index, [key, value]] of Object.entries(
				...
				// Adds the information to the list
				infoList.push(
					`${parseInt(index) + 1}. ${utils.hyperlink(key, value)}`,
				);
			}
		}
