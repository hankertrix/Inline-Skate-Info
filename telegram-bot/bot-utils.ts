...
function splitMessage(
	...
	if (
		utils.removeHtml(message).length <= maxLength &&
	...
	while (endIndex < msgLen) {
		...
		const msgSegmentEndIndex =
			startIndex + lastMatchIndex + matchStr.length;
...
export function generateInlineKeyboard(
	...
	for (const button of buttons) {
		...
		inlineKeyboard.push([Markup.button.callback(button, button)]);
	}
...
export async function answerInlineQuery(
	...
	if (messages.length === 1 && totalMessageLength <= MAX_CHARACTERS) {
		...
		return await ctx.answerInlineQuery([queryReply], {
	...
	for (const msgPart of messages) {
		...
		for (const segment of segments) {
			...
			const titleWithPageNum = utils.removeHtml(
				`${title} page ${++index}`,
			);
...
export async function messageAndFileInlineQueryHandler(
	...
	const msg = `${message}${joiningSection}${files
		.map((path, index) => {
			...
			if (isHyperlinked)
				return utils.hyperlink(filenames[index], fileUrl);
...
export async function exitValidator(ctx: Scenes.WizardContext): Promise<void> {
	...
	await ctx.reply(OPERATION_CANCELLED_MSG, { ...Markup.removeKeyboard() });
...
export const cancelCommand: [
	string,
	(ctx: Scenes.WizardContext) => Promise<void>,
