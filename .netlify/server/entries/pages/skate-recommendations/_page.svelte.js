...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  $$result.css.add(css);
  return `     ${$$result.head += `<!-- HEAD_svelte-lv3cld_START -->${$$result.title = `<title>${escape(title)} - Inline Skate Info</title>`, ""}<meta name="description" content="The list of skates recommended for beginners who intend to become serious about inline skating"><!-- HEAD_svelte-lv3cld_END -->`, ""}  <main class="svelte-1tlt5mp"><header><h1 class="text svelte-1tlt5mp"${add_attribute("id", makeUrlFriendlyString(title), 0)}><!-- HTML_TAG_START -->${addSoftHyphen(title, 16)}<!-- HTML_TAG_END --></h1></header> <article><section class="preface svelte-1tlt5mp"><p class="text"><!-- HTML_TAG_START -->${skateRecsJson.preface}<!-- HTML_TAG_END --></p></section> <section class="recommendations"> ${each(Object.entries(skateRecsJson.recommendations), ([category, [recommendedSkates, listOfVideos]]) => {
